...
class Node():
    ...
    def predict_moves(self):
        ...
        predictions2 = list(move_to_model.predict(nn_input))
        probabilities2 = list(predictions2[0])
        move_to_squares = sorted(range(len(probabilities2)),
                                     key=lambda k: probabilities2[k])
...
#### Print values:
board = chess.Board()
# one_hot_board = board.position_list_one_hot()
# reshape_board = np.array(one_hot_board).reshape(1, 8, 8, 12)
#
# predictions = list(move_to_model.predict(reshape_board))
# probabilities = list(predictions[0])
# move_to_squares = sorted(range(len(probabilities)), key=lambda k: probabilities[k])
#
# predictions2 = list(move_from_model.predict(reshape_board))
# print(predictions2)
# probabilities2 = list(predictions2[0])
# print(probabilities2)
# move_from_squares = sorted(range(len(probabilities2)), key=lambda k: probabilities2[k])
# print(move_from_squares)
...
def get_material_scores(moves):
    ...
    for move in moves:
        ...
        if board.is_capture(move):
            ...
            material_score += material_values[captured_piece]

        board.push(move)
        if board.is_checkmate():
            material_score += material_values[chess.KING]
...
def get_best_moves(from_sqs_list, to_sqs_list):
    '''Matches the probabilities found in predict_moves and minimizes
    the distance between the probabilities and a legal move. Returns
    a list of legal moves, ordered by score (found by combining prediction
    score and material score).'''
    legal_moves = [str(legal) for legal in list(board.legal_moves)]
    legal_moves_numbered = [squares_to_numbers(move)
    ...
    prediction_scores = [(400 - uncertainty) for uncertainty in
                         sorted(total_uncertainties)]
    material_scores = get_material_scores(moves_ordered)
    total_scores = [p_score + m_score for p_score, m_score in
                    zip(prediction_scores, material_scores)]
...
def get_best_moves2(from_sqs_list, to_sqs_list):
    '''Matches the probabilities found in predict_moves and minimizes
    the distance between the probabilities and a legal move. Returns
    a list of legal moves, ordered by score (found by combining prediction
    score and material score).'''
    ...
    prediction_scores = [uncertainty for uncertainty in
                         sorted(total_uncertainties)]

    return ([[move, score] for score, move in
             sorted(zip(prediction_scores, moves_ordered),
                    key=lambda x: x[0], reverse=True)])


# print(get_best_moves(move_from_squares, move_to_squares))
# print(get_best_moves2(move_from_squares, move_to_squares))
